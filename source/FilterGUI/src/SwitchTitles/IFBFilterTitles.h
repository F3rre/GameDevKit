...
public:
    ...
    bool TitExists(BYTE  ID);

    bool TitColorExists(const wchar_t* title);

    void AddTitle(const wchar_t* title, BYTE ID,int Color);
    void ResetPosition();

    std::string GetGuildName();

    static  DWORD WINAPI UpdateTitles(LPVOID lpParam);
private:
    void On_BtnClick();
    void On_BtnClick1();
    void OnClick_Exit();

...
bool CIFSwitchTitle::OnCreate(long ln)
{
    ...
    //m_textbox = m_IRM.GetResObj<CIFTextBox>(BFILTERTITLES_TEXTBOX, 1);
    ...
    //m_vericalSroll = m_IRM.GetResObj<CIFVerticalScroll>(BFILTERTITLES_VSCROLL, 1);
    ...
    m_mybutton->SetText(L"Refresh");
    m_mybutton2->SetText(L"Apply");
...
void CIFSwitchTitle::OnTimer(int timerId) {
    switch (timerId) {
    case TIMER_UPDATE_PER_SECOND:
        UpdateTimings();
        break;
    }
}

void CIFSwitchTitle::UpdateTimings() {
    time_t now;
    time(&now);
...
void CIFSwitchTitle::On_BtnClick()
{
    ...
    //if (RefreshTimes > 2)
    //{
    //    g_pCGInterface->ShowMessage_Warning(L"Titles automatically refresh every 5 minute");
    //    return;
    //}

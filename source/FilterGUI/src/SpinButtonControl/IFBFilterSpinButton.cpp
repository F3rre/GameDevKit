...
void CIFSpinControl::OnPrev() {
    ...
    case EventTime:
    {
        ...
        if (m_EventPage > 0)
        {
            ...
            CIFMainMenu::EventTimeGUI->ShowList(m_EventPage);
        }
        else
        {
            m_EventPage = 1;
...
void CIFSpinControl::OnNext() {
    ...
    case EventTime:
    {
        if (CIFMainMenu::EventTimeGUI->IsPageFull(m_EventPage, 39))
        {
            m_EventPage++;

            if (CIFMainMenu::EventTimeGUI->IsPageFull(m_EventPage, 30))
            {
                m_valueCurrent = m_EventPage;
                CIFMainMenu::EventTimeGUI->ShowList(m_EventPage);
            }
